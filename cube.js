...
// if ((camera.position.z > hypotenuse || e.deltaY > 0) && (camera.position.z < cameraMaxLength - hypotenuse || e.deltaY < 0)) {
// camera.position.z += 2 * e.deltaY;
// }
// });
...
container.addEventListener('mouseclick', function(e){
    var tween = new TWEEN.Tween( group.rotation ).to( {  x: group.rotation.x + Math.PI / 2}, 1000 ).easing( TWEEN.Easing.Quadratic.EaseOut).start();
});
...
function onMouseClick(event) {
    ...
    var intersects = raycaster.intersectObjects(sameArrayButOneDimention);
    if (intersects.length) {
        console.log(intersects[0].object.position);
    }
    //console.log(intersects);
    //console.log(sameArrayButOneDimention);
}
